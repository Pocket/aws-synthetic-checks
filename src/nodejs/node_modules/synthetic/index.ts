...
const synthetics = require("../@aws-cdk/aws-synthetics/Synthetics");
  ...
  await synthetics.executeStep(stepName, async function () {
    ...
    if (response) {
      ...
      if (status < 200 || status > 299) {
        throw new Error(
          `Non-2XX response: ${sanitizedUrl} ${status} ${statusText}`
        );
      }

      if (responseText !== responseCheck) {
        throw new Error(
          `Not expected response. Expected: ${responseCheck} Received: ${responseText}`
        );
      }

      log.info(
        `Response: ${sanitizedUrl}  Status: ${status}  Status Text: ${statusText}`
      );
    } else {
      ...
      res.on("end", () => {
        ...
        if (queryJmespath === "") {
          ...
          if (response !== responseCheck) {
            throw new Error(
              `Not expected response. Received: ${response} Expected: ${responseCheck}`
            );
          }
        } else {
...
// eventually we can split these two handlers into separate files and helper functions in a helper file
